...
# Setting up the environment
    ...
    # 3. Function to get all the default values of the Raspberry Pi
        function get_default_values() {
            ...
            # Get the Rpi number ($nbr), model ($model) and revision ($rev).
            $nbr=$(cat /proc/cpuinfo | grep -i serial | cut -d' ' -f2)
            $model=$(cat /proc/cpuinfo | grep -i hardware | cut -d' ' -f2)
            $rev=$(cat /proc/cpuinfo | grep -i revision | cut -d' ' -f2)
            # Get the CPU ($cpu) and RAM ($ram) of the Raspberry Pi.
            $cpu=$(cat /proc/cpuinfo | grep -i hardware | cut -d' ' -f3)
            $ram=$(free -h | grep -i mem | cut -d' ' -f2)
            ...
            sed -i "s/\$nbr/\033[32m$nbr\033[0m/" sump_logfile.log
            sed -i "s/\$model/\033[32m$model\033[0m/" sump_logfile.log
            sed -i "s/\$rev/\033[32m$rev\033[0m/" sump_logfile.log
            sed -i "s/\$cpu/\033[32m$cpu\033[0m/" sump_logfile.log
            sed -i "s/\$ram/\033[32m$ram\033[0m/" sump_logfile.log
        ...
        function ask_user(){
            # 1. Ask if user have a settings file. 
                ...
                if [ "$answer" = "Y" ] || [ "$answer" = "y" ]; then
                    ...
                    # Replace "Keyboard_FileName" in sump_logfile.log with the name of the file downloaded
                    sed -i "s/Keyboard_FileName/$url_param/" sump_logfile.log
                else
                # Replace "Keyboard_FileName" in sump_logfile.log with "Keyboard"
                sed -i "s/Keyboard_FileName/Keyboard/" sump_logfile.log
